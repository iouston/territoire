...
class modTerritoire extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		// Export list of local authorities
		$r++;
		...
		$this->export_sql_start[$r] ='SELECT localauthoritycode, localauthorityname';collectivite
		...
		// Export list of towns, geographical code, zipcode, local authority,
		//local authority code, name of the department, number of the department, region,
		//number of the region
		$r++;
		$this->export_code[$r]=$this->rights_class.'_'.$r;
		$this->export_label[$r]='ExportCollecEtVille';
		$this->export_icon[$r]=$this->picto;
		$this->export_permission[$r]=array(array("societe","export"));
		$this->export_fields_array[$r]=array(
			'r.code_region'=>"Region_Code",
			'r.nom' => "Region_Name",
			'd.code_departement' => "Department",
			'z.code' => "Geographical_Code",
			'z.zip' => "Zipcode",
			'z.town' => "Town",
			'tc.fk_localauthoritycode' => "Local_Authority_Code",
			'collec.localauthorityname' => "Local_Authority_Name",
		);
		$this->export_TypeFields_array[$r]=array(
			'r.code_region'=>"Text",
			'r.nom' => "Text",
			'd.code_departement' => "Text",
			'z.code' => "Text",
			'z.zip' => "Text",
			'z.town' => "Text",
			'tc.fk_localauthoritycode' => "Text",
			'collec.localauthorityname' => "Text",
		);
		$this->export_sql_start[$r]='SELECT r.code_region, r.nom, d.code_departement, d.fk_region, d.nom, z.code, z.zip, z.town, tc.fk_code, tc.fk_localauthoritycode, collec.localauthoritycode, collec.localauthorityname';
		$this->export_sql_end[$r] = ' FROM '.MAIN_DB_PREFIX.'c_regions as r';
		$this->export_sql_end[$r] .= ' LEFT JOIN '.MAIN_DB_PREFIX.'c_departements as d';
		$this->export_sql_end[$r] .= ' ON r.code_region = d.fk_region';
		$this->export_sql_end[$r] .= ' LEFT JOIN '.MAIN_DB_PREFIX.'c_ziptown as z';
		$this->export_sql_end[$r] .= ' ON SUBSTRING(z.zip, 1, 2) = d.code_departement';
		$this->export_sql_end[$r] .= ' LEFT JOIN '.MAIN_DB_PREFIX.'c_territoire_collecparville as tc';
		$this->export_sql_end[$r] .= ' ON z.code = tc.fk_code';
		$this->export_sql_end[$r] .= ' LEFT JOIN '.MAIN_DB_PREFIX.'c_territoire_collectivites as collec';
		$this->export_sql_end[$r] .= ' ON collec.localauthoritycode = tc.fk_localauthoritycode';
		$this->export_sql_end[$r] .= ' ORDER BY z.town ASC';
