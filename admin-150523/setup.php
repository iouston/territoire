...
if ($action == $acts[0])
{
    $sql = "UPDATE ".MAIN_DB_PREFIX."c_transporteurs SET active = 1 WHERE rowid='".$rowid."'";
...
if ($action == $acts[1])
{
    $sql = "UPDATE ".MAIN_DB_PREFIX."c_transporteurs SET active = 0 WHERE rowid='".$rowid."'";
...
if($action == 'del_ACTIVE')
{
    echo 'TEST ***********************';
    $rowidactive = GETPOST('rowidactive', 'alpha');
    $sql = 'UPDATE ' . MAIN_DB_PREFIX.'c_territoire_collectivites';
    $sql .= 'SET active = 1';
    $sql .= 'WHERE rowid =' . $rowidactive;
    $resql = $db->query($sql);
...
// //test fetch_from_city_name($cityName='')
// $territoire = new territoire($db);
// $territoire->fetch_from_city_name('CHAMPENOUX');
// print 'résultat = ' . $territoire->localauthorityname;
...
print '<td><input type="text" name="active" value="" placeholder="'.$langs->trans('activeNumber').'"></td>';
    ...
    if ($num)
    {
        ...
        while ($i < $num)
        {
            ...
            if($i < 4)
            {
                print '<form action="'.$_SERVER["PHP_SELF"].'" method="POST">';
                print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
                if($action == 'del_ACTIVE' && $rowid == $i)
                {
                    echo 'TEST del ***********************';
                }

                if($action == 'set_ACTIVE' && $rowid == $i)
                {
                    echo 'TEST set ***********************';
                }
                ...
                {
                    print ajax_constantonoff('ACTIVE');
                }
                else
                {
                    if (empty($conf->global->ACTIVE))
                    {
                        print '<a href="'.$_SERVER['PHP_SELF'].'?action=set_ACTIVE&rowid='.$obj->rowid.'">'.img_picto($langs->trans("Disabled"),'switch_off').'</a>';
                        echo 'TEST 2 ********';
                    ...
                    {
                        print '<a href="'.$_SERVER['PHP_SELF'].'?action=del_ACTIVE&rowid='.$obj->rowid.'">'.img_picto($langs->trans("Enabled"),'switch_on').'</a>';
                        echo 'TEST 3 ********';
                     }
                }
                print '</td>';
            ...
            print '<td align="left"><a href="'.$url.'action='.$acts[$obj->active].'">'.$actl[$obj->active].'</a></td>';

            print '<td>';
            print '<a href="'.$_SERVER['PHP_SELF'].'?action=update&rowid='.$obj->rowid.'&token='.newtoken().'">'.img_edit().'</a>';
